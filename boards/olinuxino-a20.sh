...
postbuild() {
	...
	pushd $R/tmp/kernels/$device_name/u-boot
		make distclean
		make CROSS_COMPILE=$compiler A20-OLinuXino-Lime2_defconfig
...
build_kernel_armhf() {
	...
	pushd $R/tmp/kernels/$device_name/${device_name}-linux
		...
		sudo ${=cp} arch/arm/boot/zImage $strapdir/boot/
		sudo ${=cp} arch/arm/boot/dts/sun7i-a20-olinuxino-lime2.dtb $strapdir/boot/
	popd
